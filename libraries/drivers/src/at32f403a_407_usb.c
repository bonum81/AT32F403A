...
void usb_write_packet(uint8_t *pusr_buf, uint16_t offset_addr, uint16_t nbytes)
{
  ...
  uint32_t *d_addr = (uint32_t *)(offset_addr * 2 + g_usb_packet_address);
  
  uint32_t nhbytes = (nbytes + 1) >> 1;
  uint32_t n_index;
  for(n_index = 0; n_index < nhbytes; n_index ++)
  {
    *d_addr++ = *((uint16_t*)pusr_buf);
    pusr_buf += 2;
  }
}
...
{
  uint32_t *s_addr = (uint32_t *)(offset_addr * 2 + g_usb_packet_address);
  uint32_t nhbytes = (nbytes + 1) >> 1;
  uint32_t n_index;
  for(n_index = 0; n_index < nhbytes; n_index ++)
  {
    *((uint16_t*)pusr_buf) = (uint16_t)*s_addr ++;
    pusr_buf += 2;
  }
}
